...
- `artist_detail.html`: 出演履歴と今後のスケジュールを表示  
...
| 出演履歴表示       | ✅ 完了（artist_detail に過去出演を表示） |
| テスト整備         | ✅ 完了（不要なテスト削除＋カバレッジ96%達成） |
...
- アーティスト詳細ページにて、過去の出演履歴と今後のスケジュールを分離表示  
- イベント名・日付・ステージ・時間帯を含めて一覧表示  
- `Performance` モデルを `event_day__date` でフィルタ・ソート  
